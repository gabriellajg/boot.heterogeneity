...
In the presence of moderators, the function above tests whether the variability in the observed standardized mean differences (d) after accounting for the moderators included in the model is larger than sampling variability alone (Viechtbauer, 2010).

* In the first line, the Q-statistic is Q(df = 11) = 31.85 and the associated p-value is 0.0008. This statistic is significant (sig) at a alpha level of 0.05, meaning that the true effect sizes after accounting for the moderators are heterogeneous. 

* In the second line, the statistic following MC-ML-LRT is 5.19 and the Monte-Carlo based p-value is 0.0004. This means that the true effect sizes after accounting for the moderators are heterogeneous at a alpha level of 0.05. 

* In the third line, the statistic following MC-REML-LRT is 9.28 and the Monte-Carlo based p-value is 0.0004. This means that the true effect sizes after accounting for the moderators are heterogeneous at a alpha level of 0.05. 
...
* In the first line, the Q-statistic is Q(df = 12) = 29.06 and the associated p-value is 0.004. This statistic is significant (sig) at a alpha level of 0.05, meaning that the true effect sizes are heterogeneous. 

* In the second line, the statistic following MC-ML-LRT is 5.20 and the Monte-Carlo based p-value is 0.004. This means that the true effect sizes are heterogeneous at a alpha level of 0.05. 

* In the third line, the statistic following MC-REML-LRT is 6.13 and the Monte-Carlo based p-value is 0.0004. This means that the true effect sizes are heterogeneous at a alpha level of 0.05. 
...
* In the first line, the Q-statistic is Q(df = 25) = 34.87 and the associated p-value is 0.091. This statistic is not significant (n.s) at a alpha level of 0.05, meaning that the assumption of homogeneity cannot be rejected. 

* In the second line, the statistic following MC-ML-LRT is 2.56 and the Monte-Carlo based p-value is 0.022. This means that the assumption of homogeneity is rejected and the true effect sizes are heterogeneous at a alpha level of 0.05. 

* In the third line, the statistic following MC-REML-LRT is 3.07 and the Monte-Carlo based p-value is 0.022. This means that the assumption of homogeneity is rejected and the true effect sizes are heterogeneous at a alpha level of 0.05. 
